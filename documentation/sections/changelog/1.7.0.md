...
*   When requiring EvelentScript files in Node you must now explicitly register the compiler. This can be done with `require 'evelentscript/register'` or `EvelentScript.register()`. Also for configuration such as Mocha’s, use **evelentscript/register**.
...
*   The `options` object passed to EvelentScript is no longer mutated.
