...
*   Literate EvelentScript is now parsed entirely based on indentation, similar to the 1.x implementation; there is no longer a dependency for parsing Markdown. See [breaking changes for Literate EvelentScript parsing](#breaking-changes-literate-evelentscript).
*   JavaScript reserved words used as properties are no longer wrapped in quotes.
*   `require('evelentscript')` should now work in non-Node environments such as the builds created by Webpack or Browserify. This provides a more convenient way to include the browser compiler in builds intending to run in a browser environment.
...
*   If `$XDG_CACHE_HOME` is set, the REPL `.es_history` file is saved there.
