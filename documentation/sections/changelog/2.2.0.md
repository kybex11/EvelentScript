...
*   An error is thrown when the `es` command is run in an environment that doesn’t support some ES2015 JavaScript features that the EvelentScript compiler itself requires. This can happen if EvelentScript is installed in Node older than version 6.
