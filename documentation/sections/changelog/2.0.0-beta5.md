...
*   Node 6 is now supported, and we will try to maintain that as the minimum required version for EvelentScript 2 via the `es` command or Node API. Older versions of Node, or non-evergreen browsers, can compile via the [legacy browser compiler](./browser-compiler-legacy/evelentscript.js).
...
*   Bugfixes for source maps generated via the Node API; and stack trace line numbers when compiling EvelentScript via the Node API from within a `.es` file.
