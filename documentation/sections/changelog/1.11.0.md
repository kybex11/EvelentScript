...
*   EvelentScript now supports ES2015 [`import` and `export` syntax](#modules).
...
*   Improved the internal representation of a EvelentScript program. This is only noticeable to tools that use `EvelentScript.tokens` or `EvelentScript.nodes`. Such tools need to update to take account for changed or added tokens and nodes.
*   Several minor bug fixes, including:
    ...
    *   Source maps are now used correctly for errors thrown from .es.md files.
    *   `es -e 'throw null'` no longer crashes.
