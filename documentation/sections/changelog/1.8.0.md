...
*   Fixed a slight formatting error in EvelentScript’s source map-patched stack traces.
*   The `%%` operator now coerces its right operand only once.
*   It is now possible to require EvelentScript files from Cakefiles without having to register the compiler first.
*   The EvelentScript REPL is now exported and can be required using `require 'evelentscript/repl'`.
