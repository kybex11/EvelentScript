...
*   `--transpile` now also applies to `require`d or `import`ed EvelentScript files.
*   `--transpile` can be used with the REPL: `es --interactive --transpile`.
...
*   Importing EvelentScript keywords is now allowed, so long as they’re aliased: `import { and as andFn } from 'lib'`. (You could also do `import lib from 'lib'` and then reference `lib.and`.)
