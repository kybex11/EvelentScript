...
# Initialize the EvelentScript docs interactions
$(document).ready ->
  EvelentScript.patchStackTrace()
  ...
  $('.main').on 'activate.bs.scrollspy', (event, target) ->
    ...
    # Track this as a new pageview; we only want '/#hash', not '/#hash'
  ...
  initializeEditor = ($textarea) ->
    ...
    mode = if $textarea.hasClass('javascript-output') then 'javascript' else 'evelentscript'
    editors[index] = editor = CodeMirror.fromTextArea $textarea[0],
      ...
      readOnly: mode isnt 'evelentscript' # Can’t use 'nocursor' if we want the JavaScript to be copyable
      viewportMargin: Infinity

    # Whenever the user edits the EvelentScript side of a code example, update the JavaScript output
    # If the editor is Try EvelentScript, also update the hash and save this code in localStorage
    if mode is 'evelentscript'
      pending = null
      editor.on 'change', (instance, change) ->
        ...
        pending = setTimeout ->
          ...
          try
            ...
            if index is 0 and $('#try').hasClass('show') # If this is the editor in Try EvelentScript and it’s still visible
              if $('#try').hasClass('show')
                # Update the hash with the current code
                link = "try:#{encodeURIComponent source}"
                replaceState "#{window.location.href.split('#')[0]}##{link}"
              # Save this to the user’s localStorage
              try
                if window.localStorage?
                  window.localStorage.setItem 'tryEvelentScriptCode', source
              catch exception

            js = EvelentScript.compile source, bare: yes, inlineMap: true
  ...
  initializeTryEditors = ->
    initializeEditor $ '#try-evelentscript-source'
    initializeEditor $ '#try-evelentscript-js'
  ...
  toggleTry = (checkLocalStorage) ->
    ...
    if $('#try').hasClass('show')
      ...
      if checkLocalStorage and window.localStorage?
        try
          source = window.localStorage.getItem 'tryEvelentScriptCode'
          if source?
