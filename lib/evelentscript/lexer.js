...
(function() {
  ...
  exports.Lexer = Lexer = class Lexer {
    ...
    tokenize(code, opts = {}) {
      ...
      this.literate = opts.literate; // Are we lexing literate EvelentScript?
    ...
    identifierToken() {
      ...
      if (id === 'as' && this.seenImport) {
        ...
        } else if (ref = this.value(true), indexOf.call(ES_KEYWORDS, ref) >= 0) {
          prev = this.prev();
      ...
      if (id === 'as' && this.seenExport) {
        ...
        if (ref3 = this.value(true), indexOf.call(ES_KEYWORDS, ref3) >= 0) {
          prev = this.prev();
          [prev[0], prev[1]] = ['IDENTIFIER', this.value(true)];
          this.token('AS', id);
          return id.length;
      ...
      prev = this.prev();
      tag = colon && (prev != null) && ((ref5 = prev[0]) === '(' || ref5 === 'PARAM_START' || ref5 === ',') ? 'IDENTIFIER' : colon || (prev != null) && (((ref6 = prev[0]) === '.' || ref6 === '?.' || ref6 === '::' || ref6 === '?::') || !prev.spaced && prev[0] === '@') ? 'PROPERTY' : 'IDENTIFIER';
      tokenData = {};
      if (tag === 'IDENTIFIER' && (indexOf.call(JS_KEYWORDS, id) >= 0 || indexOf.call(ES_KEYWORDS, id) >= 0) && !(this.exportSpecifierList && indexOf.call(ES_KEYWORDS, id) >= 0)) {
        if (id === 'void' && (prev != null ? prev[0] : void 0) === ':') {
          tag = 'IDENTIFIER';
        } else {
          tag = id.toUpperCase();
        }
        ...
        tag = 'FORFROM';
        this.seenFor = false;
      } else if (tag === 'IDENTIFIER' && id === 'type' && isTypeAliasStart(prev) && /^\s*[A-Za-z_$]/.test(this.chunk)) {
        tag = 'TYPE';
      ...
      if (tag === 'IDENTIFIER' && indexOf.call(RESERVED, id) >= 0 && !inJSXTag && !(id === 'void' && (prev != null ? prev[0] : void 0) === ':')) {
        this.error(`reserved word '${id}'`, {
          length: id.length
        });
      }
      if (!(tag === 'PROPERTY' || this.exportSpecifierList || this.importSpecifierList)) {
        if (indexOf.call(ES_ALIASES, id) >= 0) {
          alias = id;
          id = ES_ALIAS_MAP[id];
    ...
    literalToken() {
      ...
      if (value === ';') {
        ...
        tag = 'MATH';
      } else if (value === '<' && this.typeGenericStart()) {
        tag = 'TYPE_ARG_START';
      } else if (value === '>' && this.typeArgDepth > 0) {
        tag = 'TYPE_ARG_END';
      ...
      if (value === ':' && (prev != null ? prev[0] : void 0) === ')') {
        this.tagTypeParameters();
      }
      if (tag === 'TYPE_ARG_START') {
        this.typeArgDepth += 1;
      } else if (tag === 'TYPE_ARG_END') {
        this.typeArgDepth -= 1;
      }
    ...
    tagParameters() {
      ...
      this.convertParenGroupToParams();
      return this;
    }
    ...
    tagTypeParameters() {
      var ref;
      if (((ref = this.prev()) != null ? ref[0] : void 0) !== ')') {
        return this;
      }
      this.convertParenGroupToParams();
      return this;
    ...
    typeGenericStart() {
      var pp, prev, ref;
      prev = this.prev();
      if (this.typeArgDepth > 0) {
        return true;
      }
      if (!((prev != null ? prev[0] : void 0) === 'IDENTIFIER' && !prev.spaced)) {
        return false;
      }
      pp = (ref = this.tokens[this.tokens.length - 2]) != null ? ref[0] : void 0;
      return pp === 'INTERFACE' || pp === 'TYPE' || pp === ',' || pp === 'TYPE_ARG_END' || pp === ':' || pp === '.' || pp === 'IDENTIFIER' || pp === '|' || pp === '&' || pp === 'PARAM_START';
    }
  ...
  isUnassignable = function(name, displayName = name) {
    switch (false) {
      case indexOf.call([...JS_KEYWORDS, ...ES_KEYWORDS], name) < 0:
  ...
  isTypeAliasStart = function(prev) {
    var ref;
    if (!prev) {
      return true;
    }
    return ((ref = prev[0]) === 'TERMINATOR' || ref === 'INDENT' || ref === 'OUTDENT' || ref === 'EXPORT' || ref === 'STATEMENT') || prev.newLine;
  };
  ...
  ES_KEYWORDS = ['undefined', 'Infinity', 'NaN', 'then', 'unless', 'until', 'loop', 'of', 'by', 'when', 'interface', 'readonly'];
  ...
  RESERVED = ['case', 'function', 'var', 'void', 'with', 'const', 'let', 'enum', 'native', 'implements', 'package', 'private', 'protected', 'public', 'static'];
  ...
  INSIDE_JSX = /^(?:[^\{<])*/; // Start of EvelentScript interpolation. // Similar to `HEREDOC_DOUBLE` but there is no escaping.
  // Maybe JSX tag (`<` not allowed even if bare).

  JSX_INTERPOLATION = /^(?:\{|<(?!\/))/; // EvelentScript interpolation.
  // JSX opening tag.
