...
(function() {
  ...
  exports.Rewriter = Rewriter = (function() {
    class Rewriter {
      ...
      rewrite(tokens1) {
        var ref, ref1, t;
        this.tokens = tokens1;
        this.interfaceBodyDepth = 0;
        this.pendingInterface = false;
      ...
      addImplicitBracesAndParens() {
        ...
        return this.scanTokens(function(token, i, tokens) {
          ...
          if (tag === 'OUTDENT') {
            if (this.interfaceBodyDepth > 0) {
              this.interfaceBodyDepth -= 1;
            }
          }
          if (tag === 'INTERFACE') {
            this.pendingInterface = true;
          }
          if (tag === 'INDENT') {
            if (this.pendingInterface) {
              this.interfaceBodyDepth += 1;
              this.pendingInterface = false;
            }
          }
          ...
          if (indexOf.call(IMPLICIT_FUNC, tag) >= 0 && this.indexOfTag(i + 1, 'INDENT') > -1 && this.looksObjectish(i + 2) && !this.findTagsBackwards(i, ['CLASS', 'EXTENDS', 'IF', 'CATCH', 'SWITCH', 'LEADING_WHEN', 'FOR', 'WHILE', 'UNTIL', 'INTERFACE', 'TYPE']) && !(((ref1 = (s = (ref2 = stackTop()) != null ? ref2[0] : void 0)) === '{' || ref1 === '[') && !isImplicit(stackTop()) && this.findTagsBackwards(i, s))) {
            startImplicitCall(i + 1);
          ...
          if (tag === ':') {
            if (this.interfaceBodyDepth > 0 || (this.findTagsBackwards(i, ['PARAM_START']) && !this.findTagsBackwards(i, ['PARAM_END'])) || this.tag(i - 1) === 'PARAM_END') {
              return forward(1);
            }
  ...
  IMPLICIT_CALL = ['IDENTIFIER', 'JSX_TAG', 'PROPERTY', 'NUMBER', 'INFINITY', 'NAN', 'STRING', 'STRING_START', 'REGEX', 'REGEX_START', 'JS', 'NEW', 'PARAM_START', 'CLASS', 'INTERFACE', 'TYPE', 'IF', 'TRY', 'SWITCH', 'THIS', 'DYNAMIC_IMPORT', 'IMPORT_META', 'NEW_TARGET', 'UNDEFINED', 'NULL', 'BOOL', 'UNARY', 'DO', 'DO_IIFE', 'YIELD', 'AWAIT', 'UNARY_MATH', 'SUPER', 'THROW', '@', '->', '=>', '[', '(', '{', '--', '++'];
