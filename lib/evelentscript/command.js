...
(function() {
  // The `es` utility. Handles command-line compilation of EvelentScript
  ...
  // Allow EvelentScript to emit Node.js events.
  helpers.extend(EvelentScript, new EventEmitter());
  ...
  BANNER = `Usage: es [options] path/to/script.es [args]

If called without options, \`es\` will run your script.`;

  // The list of all the valid option flags that `es` knows how to handle.
  SWITCHES = [['--ast', 'generate an abstract syntax tree of nodes'], ['-b', '--bare', 'compile without a top-level function wrapper'], ['-c', '--compile', 'compile to JavaScript and save as .js files'], ['-e', '--eval', 'pass a string from the command line as input'], ['-h', '--help', 'display this help message'], ['-i', '--interactive', 'run an interactive EvelentScript REPL'], ['-j', '--join [FILE]', 'concatenate the source EvelentScript before compiling'], ['-l', '--literate', 'treat stdio as literate style evelentscript'], ['-m', '--map', 'generate source map and save as .js.map files'], ['-M', '--inline-map', 'generate source map and include it directly in output'], ['-n', '--nodes', 'print out the parse tree that the parser produces'], ['--nodejs [ARGS]', 'pass options directly to the "node" binary'], ['--no-header', 'suppress the "Generated by" header'], ['-o', '--output [PATH]', 'set the output path or path/filename for compiled JavaScript'], ['-p', '--print', 'print out the compiled JavaScript'], ['-r', '--require [MODULE*]', 'require the given module before eval or REPL'], ['-s', '--stdio', 'listen for and compile scripts over stdio'], ['-t', '--transpile', 'pipe generated JavaScript through Babel'], ['--tokens', 'print out the tokens that the lexer/rewriter produce'], ['-v', '--version', 'display the version number'], ['-w', '--watch', 'watch scripts for changes and rerun commands']];
  ...
  exports.buildESOptionParser = buildESOptionParser = function() {
    return new optparse.OptionParser(SWITCHES, BANNER);
  };

  // Run `es` by parsing passed options and determining what action to take.
  // Many flags cause us to divert before compiling anything. Flags passed after
  // `--` will be passed verbatim to your script as arguments in `process.argv`
  exports.run = function() {
    var err, i, len, literals, outputBasename, ref, replCliOpts, results, source;
    if (process.argv[2] === 'build') {
      return require('./build').run(process.argv.slice(3));
    }
    ...
    if ((!opts.doubleDashed) && (opts.arguments[1] === '--')) {
      printWarn(`es was invoked with '--' as the second positional argument, which is
...
replace...
    ...
    $ es --compile --join bundle.js -- a.es b.es c.es

with...

    $ cat a.es b.es c.es | es --compile --stdio > bundle.js
`);
    }
    ref = opts.arguments;
    results = [];
  ...
  // is passed, recursively compile all '.es', '.lites', and '.es.md'
  ...
  compilePath = function(source, topLevel, base) {
    ...
    if (stats.isDirectory()) {
      ...
      return results;
    } else if (topLevel || helpers.isEvelentScript(source)) {
      sources.push(source);
  ...
  findDirectoryIndex = function(source) {
    ...
    ref = EvelentScript.FILE_EXTENSIONS;
    for (i = 0, len = ref.length; i < len; i++) {
      ext = ref[i];
    ...
    console.error(`Missing index.es or index.lites in ${source}`);
  ...
  compileScript = function(file, input, base = null) {
    ...
    try {
      ...
      EvelentScript.emit('compile', task);
      if (opts.tokens) {
        return printTokens(EvelentScript.tokens(task.input, task.options));
      } else if (opts.nodes) {
        return printLine(EvelentScript.nodes(task.input, task.options).toString().trim());
      } else if (opts.ast) {
        compiled = EvelentScript.compile(task.input, task.options);
        return printLine(JSON.stringify(compiled, null, 2));
      } else if (opts.run) {
        EvelentScript.register();
        if (opts.prelude) {
          EvelentScript.eval(opts.prelude, task.options);
        }
        return EvelentScript.run(task.input, task.options);
        ...
        EvelentScript.emit('success', task);
        if (opts.print) {
      ...
      err = error;
      EvelentScript.emit('failure', err, task);
      if (EvelentScript.listeners('failure').length) {
        return;
      }
  ...
  // Watch a source EvelentScript file using `fs.watch`, recompiling it every
  ...
  // The compile-time options to pass to the EvelentScript compiler.
  compileOptions = function(filename, base) {
    var answer, cwd, jsDir, jsPath;
    if (opts.transpile) {
      try {
        // The user has requested that the EvelentScript compiler also transpile
        ...
        // run EvelentScript’s output without transpilation (modern Node or evergreen
        ...
        try {
          ...
          // Give appropriate instructions depending on whether `es` was run
          // locally or globally.
...
See /#transpilation`);
            ...
            console.error(`To use --transpile with globally-installed EvelentScript, you must have @babel/core installed globally:
...
See /#transpilation`);
      ...
      // require Babel.
      opts.transpile.transpile = EvelentScript.transpile;
  ...
  version = function() {
    return printLine(`EvelentScript version ${EvelentScript.VERSION}`);
  };
