...
# Helper function for extracting code from Literate EvelentScript by stripping
# out all non-code blocks, producing a string of EvelentScript code that can
...
# A `.es.md` compatible version of `basename`, that returns the file sans-extension.
exports.baseFileName = (file, stripExt = no, useWinPathSep = no) ->
  ...
  parts = file.split('.')
  parts.pop()
  parts.pop() if parts[parts.length - 1] is 'es' and parts.length > 1
  parts.join('.')

# Determine if a filename represents a EvelentScript file.
exports.isEvelentScript = (file) -> /\.((lit)?es|es\.md)$/.test file

# Determine if a filename represents a Literate EvelentScript file.
exports.isLiterate = (file) -> /\.(lites|es\.md)$/.test file
...
exports.throwSyntaxError = (message, location) ->
  ...
  # compile EvelentScript for example).
