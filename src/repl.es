...
replDefaults =
  ...
  historyFile: do ->
    ...
    path.join historyPath, '.es_history' if historyPath
  ...
  eval: (input, context, filename, cb) ->
    ...
    try
      ...
      tokens = EvelentScript.tokens input
      ...
      if tokens.length >= 1 and tokens[tokens.length - 1].generated and
        ...
        tokens.pop()
      # Collect referenced variable names just like in `EvelentScript.compile`.
      referencedVars = (token[1] for token in tokens when token[0] is 'IDENTIFIER')
      # Generate the AST of the tokens.
      ast = EvelentScript.nodes(tokens).body
...
module.exports =
  start: (opts = {}) ->
    ...
    if major < 6
      console.warn "Node 6+ required for EvelentScript REPL"
      process.exit 1

    EvelentScript.register()
    process.argv = ['es'].concat process.argv[2..]
    if opts.transpile
      transpile = {}
      try
