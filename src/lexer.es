# The EvelentScript Lexer. Uses a series of token-matching regexes to attempt
...
# are read by jison in the `parser.lexer` function defined in evelentscript.es.
...
# The Lexer class reads a stream of EvelentScript and divvies it up into tagged
...
exports.Lexer = class Lexer
  ...
  tokenize: (code, opts = {}) ->
    @literate   = opts.literate  # Are we lexing literate EvelentScript?
  ...
  # returns, etc. If we’re lexing literate EvelentScript, strip external Markdown
  ...
  # identifiers. Because EvelentScript reserves a handful of keywords that are
  ...
  identifierToken: ->
    ...
    if id is 'as' and @seenImport
      ...
      else if @value(yes) in ES_KEYWORDS
        prev = @prev()
    ...
    tag =
      if colon and prev? and prev[0] in ['(', 'PARAM_START', ',']
        'IDENTIFIER'
      else if colon or prev? and
         (prev[0] in ['.', '?.', '::', '?::'] or
    ...
    tokenData = {}
    if tag is 'IDENTIFIER' and (id in JS_KEYWORDS or id in ES_KEYWORDS) and
       not (@exportSpecifierList and id in ES_KEYWORDS)
      if id is 'void' and prev?[0] is ':'
        tag = 'IDENTIFIER'
      else
        tag = id.toUpperCase()
      ...
      tag = 'FORFROM'
      @seenFor = no
    else if tag is 'IDENTIFIER' and id is 'type' and isTypeAliasStart(prev) and /^\s*[A-Za-z_$]/.test(@chunk)
      tag = 'TYPE'
    # Throw an error on attempts to use `get` or `set` as keywords, or
    # what EvelentScript would normally interpret as calls to functions named
    ...
    if tag is 'IDENTIFIER' and id in RESERVED and not inJSXTag and
       not (id is 'void' and prev?[0] is ':')
  ...
  literalToken: ->
    ...
    else if value is '<' and @typeGenericStart()
      tag = 'TYPE_ARG_START'
    else if value is '>' and @typeArgDepth > 0
      tag = 'TYPE_ARG_END'
    ...
    @tagTypeParameters() if value is ':' and prev?[0] is ')'
    if tag is 'TYPE_ARG_START'
      @typeArgDepth += 1
    else if tag is 'TYPE_ARG_END'
      @typeArgDepth -= 1
  ...
  tagParameters: ->
    ...
    @convertParenGroupToParams()
    this

  # Convert the parenthesis group ending at the previous `)` into PARAM_START/PARAM_END.
  # Used for function types like `(id: ID): T` where there is no trailing `->`.
  tagTypeParameters: ->
    return this unless @prev()?[0] is ')'
    @convertParenGroupToParams()
    this
  ...
  convertParenGroupToParams: ->
    ...
    return this if paramEndToken[0] is 'PARAM_END'
  ...
  # `<` after a name in a type declaration begins generic type parameters.
  typeGenericStart: ->
    prev = @prev()
    return yes if @typeArgDepth > 0
    return no unless prev?[0] is 'IDENTIFIER' and not prev.spaced
    pp = @tokens[@tokens.length - 2]?[0]
    pp in ['INTERFACE', 'TYPE', ',', 'TYPE_ARG_END', ':', '.', 'IDENTIFIER', '|', '&', 'PARAM_START']
...
isUnassignable = (name, displayName = name) -> switch
  when name in [JS_KEYWORDS..., ES_KEYWORDS...]
...
# `from` isn’t a EvelentScript keyword, but it behaves like one in `import` and
...
# `type` is a keyword only when starting a type alias declaration.
isTypeAliasStart = (prev) ->
  return yes unless prev
  prev[0] in ['TERMINATOR', 'INDENT', 'OUTDENT', 'EXPORT', 'STATEMENT'] or prev.newLine
...
# Keywords that EvelentScript shares in common with JavaScript.
JS_KEYWORDS = [
...
ES_ALIASES  = (key for key of ES_ALIAS_MAP)
ES_KEYWORDS = ES_KEYWORDS.concat ES_ALIASES

# The list of keywords that are reserved by JavaScript, but not used, or are
# used by EvelentScript internally. We throw an error when these are encountered,
