...
BANNER = '''
  Usage: es [options] path/to/script.es [args]
...
SWITCHES = [
  ...
  ['-i', '--interactive',       'run an interactive EvelentScript REPL']
  ['-j', '--join [FILE]',       'concatenate the source EvelentScript before compiling']
  ['-l', '--literate',          'treat stdio as literate style evelentscript']
...
exports.buildESOptionParser = buildESOptionParser = ->
...
exports.run = ->
  ...
  if (not opts.doubleDashed) and (opts.arguments[1] is '--')
    printWarn '''
      es was invoked with '--' as the second positional argument, which is
...
compilePath = (source, topLevel, base) ->
  ...
  else if topLevel or helpers.isEvelentScript source
    sources.push source
...
compileScript = (file, input, base = null) ->
  ...
  try
    ...
    if opts.tokens
      printTokens EvelentScript.tokens task.input, task.options
    else if opts.nodes
      printLine EvelentScript.nodes(task.input, task.options).toString().trim()
    else if opts.ast
      compiled = EvelentScript.compile task.input, task.options
      printLine JSON.stringify(compiled, null, 2)
    else if opts.run
      EvelentScript.register()
      EvelentScript.eval opts.prelude, task.options if opts.prelude
      EvelentScript.run task.input, task.options
    ...
    return if EvelentScript.listeners('failure').length
    ...
    try
      ...
      try
        ...
        if require.resolve('.').indexOf(process.cwd()) is 0
          ...
          console.error '''
            To use --transpile with globally-installed EvelentScript, you must have @babel/core installed globally:
...
version = ->
  printLine "EvelentScript version #{EvelentScript.VERSION}"
