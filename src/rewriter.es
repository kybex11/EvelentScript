# The EvelentScript language has a good deal of optional syntax, implicit syntax,
...
exports.Rewriter = class Rewriter
  ...
  addImplicitBracesAndParens: ->
    ...
    @scanTokens (token, i, tokens) ->
      ...
      if tag is 'OUTDENT'
        @interfaceBodyDepth -= 1 if @interfaceBodyDepth > 0

      if tag is 'INTERFACE'
        @pendingInterface = yes

      if tag is 'INDENT'
        if @pendingInterface
          @interfaceBodyDepth += 1
          @pendingInterface = no
      ...
      if (tag in IMPLICIT_FUNC and token.spaced or
         ...
         (nextTag is '...' and @tag(i + 2) in IMPLICIT_CALL and not @findTagsBackwards(i, ['INDEX_START', '['])) or
          ...
          not inControlFlow() and
          not (nextTag in ['->', '=>'] and i >= 2 and @tag(i - 1) is ':' and @tag(i - 2) is 'PARAM_END')
      ...
      if tag in IMPLICIT_FUNC and
         ...
         not @findTagsBackwards(i, ['CLASS', 'EXTENDS', 'IF', 'CATCH',
          'SWITCH', 'LEADING_WHEN', 'FOR', 'WHILE', 'UNTIL', 'INTERFACE', 'TYPE']) and
      ...
      if tag is ':'
        if @interfaceBodyDepth > 0 or
           (@findTagsBackwards(i, ['PARAM_START']) and not @findTagsBackwards(i, ['PARAM_END'])) or
           @tag(i - 1) is 'PARAM_END'
          return forward(1)
...
IMPLICIT_CALL    = [
  ...
  'NEW', 'PARAM_START', 'CLASS', 'INTERFACE', 'TYPE', 'IF', 'TRY', 'SWITCH', 'THIS'
...
CONTROL_IN_IMPLICIT = ['IF', 'TRY', 'FINALLY', 'CATCH', 'CLASS', 'INTERFACE', 'TYPE', 'SWITCH']

# Tokens that are swallowed up by the parser, never leading to code generation.
# You can spot these in `grammar.es` because the `o` function second
