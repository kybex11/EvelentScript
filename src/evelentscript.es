# EvelentScript can be used both on the server, as a command-line compiler based
# on Node.js/V8, or to run EvelentScript directly in the browser. This module
# contains the main entry functions for tokenizing, parsing, and compiling
# source EvelentScript into JavaScript.
...
# evaluated from `lib/evelentscript`.
packageJson   = require '../../package.json'

# The current EvelentScript version number.
exports.VERSION = packageJson.version

exports.FILE_EXTENSIONS = FILE_EXTENSIONS = ['.es', '.lites', '.es.md']
...
withPrettyErrors = (fn) ->
  (code, options = {}) ->
    try
      ...
      throw err if typeof code isnt 'string' # Support `EvelentScript.nodes(tokens)`.
      throw helpers.updateSyntaxError err, code, options.filename

# Compile EvelentScript code to JavaScript, using the Jison compiler.
#
...
# object, where sourceMap is a sourcemap.es#SourceMap object, handy for
...
exports.compile = compile = withPrettyErrors (code, options = {}) ->
  ...
  if options.header
    header = "Generated by EvelentScript #{@VERSION}"
    js = "// #{header}\n#{js}"
...
# Tokenize a string of EvelentScript code, and return the array of tokens.
exports.tokens = withPrettyErrors (code, options) ->
  lexer.tokenize code, options

# Parse a string of EvelentScript code or an array of lexed tokens, and
...
# instead. These methods have been moved into `index.es` to provide
...
exports.run = exports.eval = exports.register = ->
  throw new Error 'require index.es, not this file'
...
exports.patchStackTrace = ->
  ...
  # Based on [michaelficarra/EvelentScriptRedux](http://goo.gl/ZTx1p)
  # NodeJS / V8 have no support for transforming positions in stack traces using
  # sourceMap, so we must monkey-patch Error to display EvelentScript source
  # positions.
  Error.prepareStackTrace = (err, stack) ->
    frames = for frame in stack
      # Don’t display stack frames deeper than `EvelentScript.run`.
