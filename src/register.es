EvelentScript  = require './'
child_process = require 'child_process'
helpers       = require './helpers'
path          = require 'path'

{patchStackTrace} = EvelentScript
...
# Load and run a EvelentScript file for Node, stripping any `BOM`s.
loadFile = (module, filename) ->
  options = module.options or getRootModule(module).options or {}

  # Currently `EvelentScript.compile` caches all source maps if present. They
  # are available in `getSourceMap` retrieved by `filename`.
  if cacheSourceMaps or nodeSourceMapsSupportEnabled
    options.inlineMap = true
  js = EvelentScript._compileFile filename, options

  module._compile js, filename

# If the installed version of Node supports `require.extensions`, register
# EvelentScript as an extension.
if require.extensions
  for ext in EvelentScript.FILE_EXTENSIONS
...
# If we're on Node, patch `child_process.fork` so that EvelentScript scripts are able
# to fork both EvelentScript files, and JavaScript files, directly.
if child_process
  {fork} = child_process
  binary = require.resolve '../../bin/es'
  child_process.fork = (path, args, options) ->
    if helpers.isEvelentScript path
